...
### Practice
1) Button Count
    1) Write a program so that when a button is pressed a statement is printed to serial monitor
    2) Write a program that outputs to serial how many times a button has been pressed
...
```
### Practice
1) Serial Input - Write a program that will turn an LED on if you type "ON" and off if you type "OFF"
2) Input Period - Write a program that allows you to input two numbers
...
1) Write a program for a voltage divider with one known resistance and one thermistor
    1) make the program return the actual voltage value that is read
    2) convert this voltage value into the temperature in celcius
    3) Make a program that lights an LED if the temperature measured exceeds 26C
